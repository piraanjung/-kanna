...
class WithdrawController extends Controller
{
    ...
    //     // return view('trashbank.withdraw.index');
    // }

    public function index(){
        $withdraws = WithdrawTranction::where('withdraw_status' , 'pending')->get();
        return view('trashbank.withdraw.index',['withdraws' => $withdraws]);
    }

    public function withdraw_form(){
        $users = DB::table('users')->select('id', 'name', 'last_name')
                ->where('status', 1)->get();
        return view('trashbank.withdraw/withdraw_form',['users' => $users]);
    }
    ...
    public function edit($id){
        ...
        return view('trashbank.withdraw.edit', compact('wd_transaction', 'users'));
    }
    ...
    public function pay_money($id){
        ...
        $wd_transaction->withdraw_date = $this->reverse_date_format($wd_transaction->withdraw_date);
        $wd_transaction->get_money_date = $this->reverse_date_format($wd_transaction->get_money_date);
        return view('trashbank.withdraw.pay_money', compact('users', 'wd_transaction','acc_saving'));
    }

    public function withdraw_history(){
        $withdraws = WithdrawTranction::where('withdraw_status', '!=' , 'pending')->get();
        return view('trashbank.withdraw.withdraw_history',compact('withdraws'));
    }

    public function print($id){
        $withdraws = WithdrawTranction::where('withdraw_status' , 'pending')->get();

        $pdf = PDF::loadView('trashbank.withdraw.withdraw_table2', compact('withdraws'));
